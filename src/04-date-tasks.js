...
function isLeapYear(date) {
  const year = date.toString().split(' ').filter((el) => el.length === 4);
  if (Number(year[0]) % 4 !== 0) {
    return false;
  } if (Number(year[0]) % 100 !== 0) {
    return true;
  } if (Number(year[0]) % 400 !== 0) {
    return false;
  }
  return true;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const millisec = (endDate.getMilliseconds() - startDate.getMilliseconds()).toString().padStart(3, '0');
